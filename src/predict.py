...
def predict_job(client, dry_run=False):
    ...
    for _ in range(max_retry_count):
        try:
            ...
            df = df.loc[
                max_timestamp - pd.to_timedelta(model.max_data_sec, unit='S') <= df.index.get_level_values('timestamp')]
            break
    ...
    if dry_run:
        ...
        for i in range(1, 13):
            ...
            client.submit(
                tournament='crypto',
                model_id=model_id,
                timestamp=int((max_timestamp + pd.to_timedelta(1, unit='H') + i * pd.to_timedelta(5, unit='minutes')).timestamp()),
